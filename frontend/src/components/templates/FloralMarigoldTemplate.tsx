...
export default function FloralMarigoldTemplate({ invite, variant = 'light' }: TemplateProps) {
  ...
  return (
    <BaseTemplate
      ...
      rsvpBackground={isDark ? 'rgba(130, 50, 5, 0.92)' : 'rgba(130, 50, 5, 0.88)'}
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden" style={{ zIndex: 5 }}>
        ...
        <div className="absolute inset-0" style={{
          background: 'radial-gradient(ellipse 65% 55% at 50% 50%, rgba(244,168,50,0.10) 0%, rgba(192,57,43,0.20) 55%, rgba(0,0,0,0.45) 100%)',
        }} />

        {/* Marigold garland — top border */}
        <svg viewBox="0 0 800 80" className="absolute top-0 left-0 w-full opacity-40" preserveAspectRatio="none" aria-hidden="true">
          <defs>
          ...
          {/* Garland string */}
          <path d="M0,20 Q50,5 100,20 Q150,35 200,20 Q250,5 300,20 Q350,35 400,20 Q450,5 500,20 Q550,35 600,20 Q650,5 700,20 Q750,35 800,20"
            fill="none" stroke={yellow} strokeWidth="1.5" opacity="0.6" />
          {/* Marigold blooms along garland */}
          {[50, 150, 250, 350, 450, 550, 650, 750].map((x, i) => (
            <g key={i} transform={`translate(${x}, ${i % 2 === 0 ? 18 : 28})`}>
              {bloomPetals.slice(0, 8).map((angle, pi) => (
                <ellipse key={pi} cx="0" cy="-10" rx="4" ry="8"
                  fill={pi % 2 === 0 ? yellow : orange}
        ...
        <svg viewBox="0 0 800 80" className="absolute bottom-0 left-0 w-full opacity-40" preserveAspectRatio="none" style={{ transform: 'scaleY(-1)' }} aria-hidden="true">
          <path d="M0,20 Q50,5 100,20 Q150,35 200,20 Q250,5 300,20 Q350,35 400,20 Q450,5 500,20 Q550,35 600,20 Q650,5 700,20 Q750,35 800,20"
            fill="none" stroke={yellow} strokeWidth="1.5" opacity="0.6" />
        ...
        <svg viewBox="0 0 300 120" className="absolute top-8 left-1/2 -translate-x-1/2 w-64 h-24 opacity-30" aria-hidden="true">
            ...
            {Array.from({ length: 9 }, (_, i) => {
              ...
              return (
                <g key={i} transform={`rotate(${angle})`}>
                  <ellipse cx="0" cy="-55" rx="10" ry="28"
                    fill={i % 2 === 0 ? yellow : orange} opacity={0.7 + (i === 4 ? 0.2 : 0)} />
        ...
        {[
          { x: 30, y: 200, scale: 0.7 }, { x: 770, y: 180, scale: 0.6 },
          { x: 20, y: 380, scale: 0.5 }, { x: 780, y: 360, scale: 0.65 },
        ].map((pos, idx) => (
          <svg key={idx} viewBox="0 0 60 60"
            className="absolute opacity-20"
            style={{ left: pos.x - 15, top: pos.y - 15, width: 40 * pos.scale, height: 40 * pos.scale }}
            aria-hidden="true">
            <g transform="translate(30,30)">
              {bloomPetals.slice(0, 6).map((angle, pi) => (
                <ellipse key={pi} cx="0" cy="-14" rx="5" ry="10"
                  fill={pi % 2 === 0 ? yellow : orange}
