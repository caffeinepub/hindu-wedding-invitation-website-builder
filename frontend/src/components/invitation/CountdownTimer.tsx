...
function FlipUnit({ value, label, accentColor }: { value: number; label: string; accentColor: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <span className="text-xs uppercase tracking-widest opacity-70 font-garamond">{label}</span>
    </div>
  );
}

export default function CountdownTimer({ weddingDate, accentColor = '#D4AF37' }: CountdownTimerProps) {
  ...
  return (
    <div className="flex items-center justify-center gap-3 md:gap-6">
      ...
      <span className="text-3xl font-bold opacity-50 mb-4" style={{ color: accentColor }}>:</span>
      <FlipUnit value={timeLeft.hours} label="Hours" accentColor={accentColor} />
      <span className="text-3xl font-bold opacity-50 mb-4" style={{ color: accentColor }}>:</span>
      <FlipUnit value={timeLeft.minutes} label="Minutes" accentColor={accentColor} />
      <span className="text-3xl font-bold opacity-50 mb-4" style={{ color: accentColor }}>:</span>
      <FlipUnit value={timeLeft.seconds} label="Seconds" accentColor={accentColor} />
    </div>
  );
}
