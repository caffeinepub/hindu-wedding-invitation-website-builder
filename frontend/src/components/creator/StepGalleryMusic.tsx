...
import { X, Music, GripVertical, ImageIcon, Loader2 } from 'lucide-react';
...
export default function StepGalleryMusic({ galleryItems, musicUrl, onGalleryChange, onMusicChange }: StepGalleryMusicProps) {
  ...
  const removeImage = (i: number) => onGalleryChange(galleryItems.filter((_, idx) => idx !== i));
  ...
  const handleMusicFile = async (file: File) => {
    const isValidType = ['audio/mpeg', 'audio/ogg', 'audio/mp3'].includes(file.type) || file.name.match(/\.(mp3|ogg)$/i);
  ...
  return (
    <div className="space-y-10">
      {/* Gallery */}
      <div>
        <h2 className="text-2xl font-cormorant font-bold text-gold mb-2">Photo Gallery</h2>
        <p className="text-ivory/50 font-garamond text-sm mb-6">Upload multiple photos. Drag to reorder. Uploaded securely in chunks.</p>

        <div
          className={cn(
            'border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all',
            dragOver ? 'border-gold bg-gold/10' : 'border-white/20 hover:border-gold/40'
          )}
        ...
        >
          ...
          <p className="text-ivory/30 text-xs mt-1">JPG, PNG, WebP · Max 20MB each</p>
        </div>
        <input
          ref={galleryInputRef}
          type="file"
            ...
            {galleryItems.map((item, i) => (
              <div
                ...
                className={cn(
                  ...
                  dragIdx === i ? 'opacity-50 scale-95' : 'border-white/10 hover:border-gold/40'
                )}
              >
                <img src={item.previewUrl} alt={`Gallery ${i + 1}`} className="w-full h-full object-cover" />
                {item.uploading && (
                  <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center gap-1">
                    <Loader2 size={16} className="text-gold animate-spin" />
                    <div className="w-10 h-1 bg-white/20 rounded-full overflow-hidden">
                      <div className="h-full bg-gold rounded-full transition-all" style={{ width: `${item.progress}%` }} />
                    </div>
                    <p className="text-white text-xs">{Math.round(item.progress)}%</p>
                  </div>
                )}
                {!item.uploading && (
                  <>
      ...
      <div className="border-t border-white/10 pt-8">
        <h3 className="text-xl font-cormorant font-bold text-gold mb-2">Background Music</h3>
        <p className="text-ivory/50 font-garamond text-sm mb-6">Optional. MP3 or OGG format. Plays after guest interaction.</p>

        <div
          className={cn(
            'border-2 border-dashed rounded-2xl p-6 text-center cursor-pointer transition-all',
            musicUrl ? 'border-gold/60 bg-gold/5' : 'border-white/20 hover:border-gold/40'
          )}
          onClick={() => musicInputRef.current?.click()}
        >
          <Music size={28} className={cn('mx-auto mb-2', musicUrl ? 'text-gold' : 'text-ivory/30')} />
          {musicUrl ? (
            <div>
              <p className="text-gold font-garamond text-sm">Music uploaded ✓</p>
              <button
                onClick={(e) => { e.stopPropagation(); onMusicChange(''); }}
