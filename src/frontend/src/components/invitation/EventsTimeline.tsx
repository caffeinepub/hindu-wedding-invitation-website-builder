    ...
    >
      ...
      <div className={`w-full md:w-5/12 ${isLeft ? "md:pr-8" : "md:pl-8"}`}>
        <GlassmorphismCard dark={dark} className="p-6">
          ...
          {event.details && (
            <p className="text-sm opacity-70 font-garamond mt-2">
              {event.details}
            </p>
          )}

          {event.details?.startsWith("http") && (
            <a
              href={event.details}
              target="_blank"
  ...
  return (
    <div className="relative">
      ...
      <div
        ...
        style={{
          background: `linear-gradient(to bottom, transparent, ${accentColor}66, transparent)`,
        }}
      />
      {events.map((event, i) => (
        <EventCard
