...
import type { ExternalBlob, InvitePayload } from "../../backend";
...
function getInitialSteps(payload: InvitePayload): UploadStep[] {
  ...
  steps.push({
    label: "Bride's photo",
    status: hasBride ? "pending" : "skipped",
    progress: 0,
  });
  steps.push({
    label: "Groom's photo",
    status: hasGroom ? "pending" : "skipped",
    progress: 0,
  });
  if (hasCover)
    steps.push({ label: "Cover image", status: "pending", progress: 0 });
  if (galleryCount > 0)
    steps.push({
      label: `Gallery (${galleryCount} photos)`,
      status: "pending",
      progress: 0,
    });
...
export default function StepPublish({
  ...
  const updateStep = (
    label: string,
    patch: Partial<UploadStep>,
    currentSteps: UploadStep[],
  ): UploadStep[] => {
    return currentSteps.map((s) =>
      s.label === label ? { ...s, ...patch } : s,
    );
  };
  ...
  const publish = async () => {
    ...
    if (brideHasPhoto && payload.bridePhoto) {
      ...
      enrichedPayload.bridePhoto = payload.bridePhoto.withUploadProgress(
        (pct) => {
          setSteps((prev) =>
            updateStep(
              "Bride's photo",
              { progress: pct, status: pct >= 100 ? "done" : "uploading" },
              prev,
            ),
          );
        },
    ...
    if (groomHasPhoto && payload.groomPhoto) {
      ...
      enrichedPayload.groomPhoto = payload.groomPhoto.withUploadProgress(
        (pct) => {
          setSteps((prev) =>
            updateStep(
              "Groom's photo",
    ...
    if (isRealBlob(payload.coverPhoto) && payload.coverPhoto) {
      ...
      enrichedPayload.coverPhoto = payload.coverPhoto.withUploadProgress(
        (pct) => {
          setSteps((prev) =>
            updateStep(
              "Cover image",
      ...
      enrichedPayload.galleryImages = payload.galleryImages.map((blob) => {
        ...
        return blob.withUploadProgress((pct) => {
          ...
          const overallPct = Math.round(
            ((completedGallery + pct / 100) / galleryCount) * 100,
          );
          setSteps((prev) =>
            updateStep(
              `Gallery (${galleryCount} photos)`,
    ...
    setSteps((prev) =>
      updateStep("Saving invitation", { status: "uploading" }, prev),
    );
  ...
  if (inviteId) {
    return (
      <div className="text-center">
        ...
        <h2 className="text-3xl font-cormorant font-bold text-gold mb-2">
          Your Invitation is Live!
        </h2>
        <p className="text-ivory/60 font-garamond mb-8">
  ...
  return (
    <div className="text-center py-8">
      ...
      <h2 className="text-3xl font-cormorant font-bold text-gold mb-3">
        Ready to Publish?
      </h2>
      <p className="text-ivory/60 font-garamond mb-8 max-w-md mx-auto">
        Your invitation will be published with a unique shareable URL and QR
        code.
      </p>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-6 mb-8 text-left max-w-sm mx-auto">
        ...
        <p className="text-ivory font-cormorant font-bold text-lg">
          {payload.coupleNames || "Couple Names"}
        </p>
        <p className="text-ivory/60 font-garamond text-sm">
          {payload.weddingDate || "Wedding Date"}
        </p>
        <p className="text-gold/70 font-garamond text-sm mt-1">
      ...
      {isUnavailable && !isPending && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 mb-6 max-w-sm mx-auto flex items-center gap-3">
          ...
          <p className="text-amber-400 font-garamond text-sm text-left">
            Still connecting to backend… you can try publishing now or wait a
            moment.
          </p>
        </div>
      )}
      ...
      {isPending && steps.length > 0 && (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-5 mb-6 max-w-sm mx-auto text-left space-y-3">
          <p className="text-ivory/50 font-garamond text-xs uppercase tracking-widest mb-3">
            Publishing progress
          </p>
          {steps.map((step) => (
            <div key={step.label} className="flex items-center gap-3">
              <div className="w-5 h-5 flex-shrink-0 flex items-center justify-center">
                {step.status === "done" || step.status === "skipped" ? (
                  <CheckCircle2
                    size={16}
                    className={
                      step.status === "done" ? "text-gold" : "text-ivory/20"
                    }
                  />
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-0.5">
                  <span
                    className={`text-xs font-garamond ${step.status === "skipped" ? "text-ivory/20" : "text-ivory/70"}`}
                  >
                    {step.label}
                  </span>
                  {step.status === "uploading" && (
                    <span className="text-xs text-gold/70 font-garamond">
                      {step.progress}%
                    </span>
                  )}
                </div>
      ...
      <Button
        ...
        style={{
          background: "linear-gradient(135deg, #D4AF37, #F4A832)",
          border: "none",
        }}
      >
        {isPending ? (
