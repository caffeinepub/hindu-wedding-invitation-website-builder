...
function PhotoUploader({
  ...
  const displayUrl =
    state.previewUrl || (currentBlob ? currentBlob.getDirectURL() : "");
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <button
        type="button"
        className={cn(
          "relative w-48 h-64 rounded-2xl border-2 border-dashed cursor-pointer overflow-hidden transition-all text-left",
          displayUrl
            ? "border-gold/60"
      ...
      >
        {displayUrl ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-3 bg-white/5">
            ...
            <p className="text-ivory/40 text-xs font-garamond text-center px-3">
              {placeholder}
            </p>
          </div>
        )}
...
export default function StepPhotoUpload({
  ...
  return (
    <div>
      ...
      <p className="text-ivory/50 font-garamond text-sm mb-8">
        Photos are uploaded securely in chunks — no size limits
      </p>
      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-12 mb-12">
        ...
        />
        <div className="text-4xl text-gold/40 font-cormorant hidden md:block">
          ❤
        </div>
        <PhotoUploader
          label="Groom's Photo"
      ...
      <div className="border-t border-white/10 pt-8">
        <h3 className="text-lg font-cormorant font-bold text-gold mb-4">
