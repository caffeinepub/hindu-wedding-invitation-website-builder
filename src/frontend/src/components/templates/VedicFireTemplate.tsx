...
export default function VedicFireTemplate({
  invite,
  variant = "light",
}: TemplateProps) {
  ...
  const heroStyle: React.CSSProperties = isDark
    ? {
        background:
          "linear-gradient(160deg, #1a0500 0%, #3d0f00 50%, #1a0500 100%)",
      }
    : {
        background:
          "linear-gradient(160deg, #7a2000 0%, #cc4400 50%, #7a2000 100%)",
      };
  ...
  return (
    <BaseTemplate
      ...
      rsvpBackground={
        isDark ? "rgba(100, 20, 5, 0.93)" : "rgba(100, 20, 5, 0.90)"
      }
    >
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse 70% 60% at 50% 70%, rgba(255,69,0,0.18) 0%, rgba(204,34,0,0.22) 45%, rgba(0,0,0,0.55) 100%)",
          }}
        />
        ...
        <svg
          viewBox="0 0 300 300"
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-72 opacity-18"
          aria-hidden="true"
        >
          <g transform="translate(150,150)">
            ...
            {[0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330].map(
              (deg) => (
                <g key={`yantra-${deg}`} transform={`rotate(${deg})`}>
                  <circle
                    cx="0"
                    cy="-140"
                    r="4"
        ...
        >
          ...
          {[
            ...
            <g key={`vf-border-${cx}`} transform={`translate(${cx}, 20)`}>
              <polygon
        ...
        >
          <path
            d="M0,80 Q20,40 40,20 Q50,10 60,0"
            stroke={fireRed}
            strokeWidth="2.5"
            fill="none"
          />
          <path
            d="M0,80 Q30,50 50,30 Q60,20 70,0"
            stroke={accentColor}
            strokeWidth="1.5"
            fill="none"
            opacity="0.7"
          />
          <path
            d="M0,80 Q10,60 20,40 Q30,25 40,0"
            stroke={fireAmber}
            strokeWidth="1"
            fill="none"
            opacity="0.5"
