...
import StepGalleryMusic, {
  type GalleryItem,
} from "../components/creator/StepGalleryMusic";
...
export default function CreateInvitePage() {
  ...
  const { data: existingInvite, isLoading: isLoadingInvite } = useGetInvite(
    editId ?? "",
  );
  ...
  useEffect(() => {
    ...
    const customFields: Record<string, string> = Object.fromEntries(
      existingInvite.customTextFields,
    );
    const tagline = customFields.tagline ?? "";

    // Gallery items — wrap existing ExternalBlobs into GalleryItem shape
    const loadedGallery: GalleryItem[] = existingInvite.galleryImages.map(
      (blob) => ({
        blob,
    ...
    setForm({
      ...
      themeVariant:
        (existingInvite.themeVariant as "light" | "dark") || "light",
      couple: {
        brideName,
        groomName,
        ...
        <div className="min-h-[80vh] flex items-center justify-center px-4">
          <div className="text-center max-w-md">
            ...
            <p className="font-raleway text-foreground/60 text-sm leading-relaxed mb-8">
              Please sign in to create or edit wedding invitations. Your
              invitations will be saved to your account.
            </p>
            <button
              type="button"
              onClick={() => login()}
      ...
      <AppLayout>
        <div className="min-h-[80vh] flex items-center justify-center px-4">
          <div className="text-center">
            ...
            <p className="font-raleway text-foreground/60 text-sm">
  ...
  const validateStep = (s: number): boolean => {
    ...
    if (s === 1) {
      if (!form.couple.brideName.trim())
        errs.brideName = "Bride's name is required";
      if (!form.couple.groomName.trim())
        errs.groomName = "Groom's name is required";
      if (!form.couple.weddingDate)
  ...
  const buildPayload = (): InvitePayload => {
    ...
    return {
      ...
      events: form.events,
      customTextFields: form.couple.tagline
        ? [["tagline", form.couple.tagline]]
        : [],
      sectionsConfig: [],
          ...
          <div className="bg-white/3 border border-white/8 rounded-3xl p-6 md:p-10 min-h-[500px]">
            ...
            {step === 2 && (
              <StepPhotoUpload
                ...
                onBrideUpload={(blob) =>
                  setForm((f) => ({ ...f, bridePhoto: blob }))
                }
                onGroomUpload={(blob) =>
                  setForm((f) => ({ ...f, groomPhoto: blob }))
                }
                onCoverUpload={(blob) =>
                  setForm((f) => ({ ...f, coverPhoto: blob }))
                }
              />
            )}
            {step === 3 && (
