...
export default function RSVPResponsesPage() {
  ...
  const { data: creatorPrincipal, isLoading: creatorLoading } =
    useInviteCreator(id);
  ...
  const isCreator =
    ...
    creatorPrincipal === currentPrincipal;
        ...
        <div className="min-h-[80vh] flex items-center justify-center px-4">
          <div className="text-center max-w-md">
            ...
            <p className="font-raleway text-foreground/60 text-sm leading-relaxed mb-8">
              Only the invitation creator can view RSVP responses. Please sign
              in to continue.
            </p>
            <button
              type="button"
        ...
        <div className="min-h-[80vh] flex items-center justify-center px-4">
          <div className="text-center max-w-md">
            ...
            <p className="font-raleway text-foreground/50 text-sm leading-relaxed mb-8">
              You are not the creator of this invitation. Only the creator can
              view RSVP responses.
            </p>
            <button
              type="button"
  ...
  const totalGuests =
    responses?.reduce((sum, r) => sum + Number(r.guests), 0) ?? 0;
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            ...
            >
              ...
              <div className={`font-cinzel text-2xl font-bold ${color} mb-1`}>
                {value}
              </div>
              <div className="font-raleway text-xs text-foreground/40 tracking-wide">
                {label}
              </div>
            </div>
          ))}
          ...
          <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-deepMaroon/10 border-b border-gold/10">
                    <th className="text-left px-5 py-3 font-cinzel text-xs text-gold/60 tracking-widest uppercase">
                      Name
                    </th>
                    <th className="text-left px-5 py-3 font-cinzel text-xs text-gold/60 tracking-widest uppercase">
                      Status
                ...
                <tbody>
                  {responses.map((r, i) => (
                    ...
                    >
                      <td className="px-5 py-4 font-raleway text-sm text-foreground/80">
                        {r.name}
                      </td>
            ...
            <div className="sm:hidden space-y-3">
              {responses.map((r, i) => (
                <div
                  key={`mresp-${r.name}-${i}`}
                  className="rounded-xl border border-gold/10 bg-card p-4"
                >
                  <div className="flex items-start justify-between mb-2">
                    <span className="font-raleway font-medium text-foreground/80">
                      {r.name}
                    </span>
                    <span
