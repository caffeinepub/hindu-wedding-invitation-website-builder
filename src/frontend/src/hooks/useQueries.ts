...
import type { InvitePayload, InviteRecord, RSVPRecord } from "../backend";
...
export function useCreateInvite() {
  ...
  return useMutation({
    mutationFn: async ({
      id,
      payload,
    }: { id: string; payload: InvitePayload }) => {
      if (!actor) {
        throw new Error(
          "Backend connection is not ready yet. Please wait a moment and try again.",
        );
      }
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["rsvpResponses", variables.id],
      });
    },
  });
}
